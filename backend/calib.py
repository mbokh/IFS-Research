...
pixelPositions = [243, 328, 439] #X axis data
wavelengths = [450, 532, 633] #Y axis data
samples = np.interp([300, 400], pixelPositions, wavelengths)
PIXEL_TO_WAVELENGTH_SCALING = (samples[1] - samples[0]) / 100
offsetToPeak = (328 - 866, 280 - 281) #532 nm peak
SPECTRA_ANGLE = 0.6 #math.degrees(math.atan2(278 - 281, 652 - 456)) According to full spectra image, it is 0.5

#Flip image left to right
fullSpectra = np.fliplr(cv2.imread("calibrationData/Cal_Thorlabs_LampScan_30umSlit000001.tif")[450:600, :]) #Horizontal strip
...
binarySpectra = cv2.threshold(gray[:, :823], 2, 255, cv2.THRESH_BINARY)[1]
binaryParticle = cv2.threshold(gray[:, 823:], 15, 255, cv2.THRESH_BINARY)[1]
combinedBinary = gray.copy()
combinedBinary[:, :823] = binarySpectra
combinedBinary[:, 823:] = binaryParticle
...
#Need to account for offset between slit and full image
MAX_WAVELENGTH = min(800, getWavelengthFromPixelPosition(spectraData[0] + spectraData[2] + (866 - particleData[4]))) / 1000000000
MIN_WAVELENGTH = max(550, getWavelengthFromPixelPosition(spectraData[0] + (866 - particleData[4]))) / 1000000000
